...
pipeline {
    environment {
        DOCKER_ID = "m0ph"
        DOCKER_IMAGE_CARTS_DB = "carts-db"
        DOCKER_TAG = "${BUILD_ID}"
        BUILD_AGENT  = ""
        NAMESPACE = "sock-shop"
    }
agent any
    stages {
        stage('Build') {
            ...
            sh 'docker build -t $DOCKER_ID/$DOCKER_IMAGE_CARTS_DB:$DOCKER_TAG .'
                   }
        }
        stage('Run') {
            steps {
                sh 'docker network create $BUILD_TAG'
                sh 'docker run -d --name $DOCKER_IMAGE_CARTS_DB --rm --network $BUILD_TAG $DOCKER_ID/$DOCKER_IMAGE_CARTS_DB:$DOCKER_TAG'
                sh 'docker stop $DOCKER_IMAGE_CARTS_DB'
